...
def estimate_cost():
    ...
    try:
        ...
        response = {**estimate, "current_balance": current_balance, "allowed": allowed}

        if not allowed:
            response[
                "message"
            ] = f"Insufficient credits. Required: {estimated_credits}, Available: {current_balance}"
...
def confirm_job():
    ...
    try:
        ...
        current_app.logger.info(
            f"User {user.email} confirmed job {job.id}, reserved {estimated_credits} credits"
        )
