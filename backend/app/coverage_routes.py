...
def create_wordlist():
        ...
        if source_type == "google_sheet" and (not words or len(words) == 0):
            ...
            try:
                ...
                words = sheets_service.fetch_words_from_spreadsheet(
                    creds, spreadsheet_id=source_ref, include_header=include_header
                )
                if not words:
    ...
    try:
        ...
        return jsonify({"wordlist": wordlist.to_dict(), "ingestion_report": ingestion_report}), 201
...
def get_wordlist(wordlist_id):
    ...
    wordlist = WordList.query.filter(
        db.and_(
            ...
            db.or_(
                WordList.owner_user_id == user_id, WordList.owner_user_id.is_(None)  # Global lists
            ),
        )
    ).first()
...
def refresh_wordlist(wordlist_id):
    ...
    try:
        ...
        refresh_report = wordlist_service.refresh_wordlist_from_source(
            wordlist, user, include_header=include_header
        )
        db.session.commit()

        return jsonify({"wordlist": wordlist.to_dict(), "refresh_report": refresh_report}), 200
    except Exception as e:
        db.session.rollback()
...
def get_coverage_cost():
    ...
    return jsonify({"cost": COVERAGE_RUN_COST, "currency": "credits"}), 200
...
def import_sentences_from_sheets():
    ...
    try:
        ...
        client_id = current_app.config.get("GOOGLE_CLIENT_ID") or request.headers.get(
            "X-Google-Client-ID"
        )
        client_secret = current_app.config.get("GOOGLE_CLIENT_SECRET") or request.headers.get(
        ...
        if expiry_val and datetime.now(timezone.utc) >= expiry_val:
            ...
            try:
                ...
                return (
                    jsonify(
                        {"error": "Failed to refresh Google authentication. Please sign in again."}
                    ),
                    401,
                )
        ...
        try:
            ...
            return (
                jsonify({"error": f"Failed to fetch sentences from Google Sheets: {str(e)}"}),
                500,
            )
...
def create_coverage_run():
    ...
    try:
        ...
        success, error_msg = CreditService.charge_coverage_run(
            user_id=user_id, coverage_run_id=coverage_run.id, amount=cost, description=description
        )
...
def get_coverage_run(run_id):
    ...
    assignments_paginated = assignments_query.paginate(
    ...
    if coverage_run.mode == "coverage":
        ...
        if not learning_set:
            ...
            for assignment in all_assignments:
                if (
                    assignment.sentence_index is not None
                    and assignment.sentence_index not in seen_sentences
                ):
                    seen_sentences.add(assignment.sentence_index)
                    ordered_assignments.append(assignment)
            ordered_assignments.sort(key=lambda a: a.sentence_index)
...
def swap_assignment(run_id):
    ...
    assignment = CoverageAssignment.query.filter_by(
        coverage_run_id=run_id, word_key=data["word_key"]
    ).first()
...
def export_coverage_run(run_id):
    ...
    if not user or not user.google_access_token:
        return (
            jsonify(
                {
                    "error": "Google OAuth not configured. Please connect your Google account in Settings."
                }
            ),
            400,
        )
    ...
    try:
        ...
        if coverage_run.mode == "coverage":
            ...
            stats = coverage_run.stats_json or {}
            learning_set = stats.get("learning_set") or _build_learning_set_from_assignments(
                assignments
            )

            rows = [["Rank", "Sentence"]]  # Header
            for entry in learning_set:
                rows.append([entry.get("rank"), entry.get("sentence_text")])
        else:
            # Filter mode: ranked sentences
            rows = [["Rank", "Sentence", "Score", "Index"]]  # Header
            sorted_assignments = sorted(
                assignments, key=lambda a: a.sentence_score or 0.0, reverse=True
            )
        ...
        # Create spreadsheet
        spreadsheet = sheets_service.create_spreadsheet_from_rows(user, sheet_name, rows)
...
def download_coverage_run(run_id):
    ...
    try:
        ...
        if coverage_run.mode == "coverage":
            ...
            for entry in learning_set:
                writer.writerow([entry.get("rank"), entry.get("sentence_text")])
        else:
            # Filter mode: ranked sentences
        ...
        if coverage_run.source_type == "history":
            ...
            if job and job.chunk_results:
                for chunk_result in job.chunk_results:
                    ...
                    sentences.extend(
                        [s.get("normalized", s.get("original", "")) for s in chunk_sentences]
                    )
            ...
            jsonify(
                {
                    ...
                    "coverage_percentage": (len(covered_words) / len(wordlist_keys) * 100)
                    if wordlist_keys
                    else 0,
...
def _generate_recommendation(categories, total_uncovered):
    ...
    if not_in_corpus > total_uncovered * 0.5:
        recommendations.append(
            f"{not_in_corpus} words ({not_in_corpus/total_uncovered*100:.0f}%) are not in your source material. Upload more novels or expand your corpus."
        )

    if only_long > total_uncovered * 0.3:
        recommendations.append(
            f"{only_long} words only appear in long sentences (9+ words). Consider adjusting the sentence length limit in future runs."
        )

    if only_short > total_uncovered * 0.2:
        recommendations.append(
            f"{only_short} words only appear in very short sentences (1-3 words). These may be function words or fragments."
        )

    if in_valid > total_uncovered * 0.2:
        recommendations.append(
            f"{in_valid} words are in valid-length sentences but weren't selected. The algorithm may need more iterations or the words might be in sentences competing with higher-scoring options."
        )

    if not recommendations:
        recommendations.append(
            "Most uncovered words are due to corpus limitations or sentence length constraints. Consider uploading additional source material."
        )
