...
class GlobalWordlistManager:
    ...
    def ensure_global_default_exists() -> WordList:
        ...
        try:
            ...
            lock_acquired = db.session.execute(
                db.text("SELECT pg_try_advisory_lock(:lock_id)"), {"lock_id": ADVISORY_LOCK_ID}
            ).scalar()
            ...
            if not lock_acquired:
                ...
                db.session.execute(
                    db.text("SELECT pg_advisory_lock(:lock_id)"), {"lock_id": ADVISORY_LOCK_ID}
                )
                ...
                # Release lock
                db.session.execute(
                    db.text("SELECT pg_advisory_unlock(:lock_id)"), {"lock_id": ADVISORY_LOCK_ID}
                )

                if existing:
                    logger.info(
                        f"Global wordlist created by another worker: {existing.name} (ID: {existing.id})"
                    )
                    return existing
                else:
                    logger.error(
                        "Lock released but no global wordlist found. This shouldn't happen."
                    )
            ...
            try:
                ...
                if existing:
                    logger.info(
                        f"Global default found during locked section: {existing.name} (ID: {existing.id})"
                    )
                    return existing
            ...
            if existing:
                logger.info(
                    f"Found global default after error: {existing.name} (ID: {existing.id})"
                )
                return existing
    ...
    def create_from_file(
        filepath: Path, name: str, set_as_default: bool = False, version: Optional[str] = None
    ) -> WordList:
        """
    ...
    def list_global_wordlists() -> List[WordList]:
        ...
        """
        return (
            WordList.query.filter_by(owner_user_id=None)
            .order_by(WordList.is_global_default.desc(), WordList.created_at.desc())
            .all()
        )
    ...
    def cleanup_duplicate_defaults() -> Dict:
        ...
        defaults = (
            WordList.query.filter_by(is_global_default=True)
            .order_by(WordList.id.asc())  # Oldest first
            .all()
        )
        ...
        for duplicate in duplicates:
            logger.info(
                f"Removing default flag from duplicate: {duplicate.name} (ID: {duplicate.id})"
            )
        ...
        logger.info(
            f"Cleanup complete. Kept {kept.name} (ID: {kept.id}), unmarked {len(duplicates)} duplicates"
        )
