...
class CreditService:
    ...
    @staticmethod
    def grant_monthly_credits(user_id: int, month: Optional[str] = None, 
                             amount: Optional[int] = None) -> CreditLedger:
        """
    ...
    @staticmethod
    def reserve_credits(user_id: int, job_id: int, amount: int, 
    ...
    @staticmethod
    def adjust_final_credits(user_id: int, job_id: int, reserved_amount: int, 
                            actual_amount: int, description: Optional[str] = None) -> CreditLedger:
        """
        ...
        entry = CreditLedger(
            ...
            job_id=job_id,
            pricing_version=PRICING_VERSION,
            description=description or f'Credit adjustment for job {job_id}: reserved {reserved_amount}, used {actual_amount}'
        )
        db.session.add(entry)
        db.session.commit()
    ...
    @staticmethod
    def refund_credits(user_id: int, job_id: int, amount: int, 
                      description: Optional[str] = None) -> CreditLedger:
        """
    ...
    def get_credit_summary(user_id: int, month: Optional[str] = None) -> Dict[str, Any]:
        ...
        entries = CreditLedger.query.filter_by(
            user_id=user_id,
            month=month
        ).all()
    ...
    @staticmethod
    def get_ledger_entries(user_id: int, month: Optional[str] = None, 
                          limit: Optional[int] = None) -> list[CreditLedger]:
        """
