...
class GoogleSheetsService:
    ...
    def fetch_words_from_spreadsheet(
        self,
        creds,
        spreadsheet_id: str,
        ...
        if len(words) < 10 and column.upper() == "B":
            ...
            if len(words_a) > len(words):
                logger.info(
                    f"Using fallback column A with {len(words_a)} words (B had {len(words)})"
                )
                words = words_a
    ...
    def export_to_sheet(
        ...
        creds,
        sentences,
        sheet_name="French Novel Sentences",
        folder_id=None,
        mode="new",
        ...
        if mode == "append" and existing_sheet_id:
            ...
            try:
                ...
                values = [
                    [i + 1, sentence] for i, sentence in enumerate(sentences, start=existing_rows)
                ]
            ...
            if headers and column_order:
                ...
                enabled_headers = sorted(
                    [h for h in headers if h.get("enabled", True)], key=lambda x: x.get("order", 0)
                )
                    ...
                    for email in sharing["collaboratorEmails"]:
                        if email and email.strip():
                            ...
                            drive_service.permissions().create(
                                fileId=spreadsheet_id, body=permission, sendNotificationEmail=False
                            ).execute()
    ...
    def create_spreadsheet_from_rows(
        self, user_or_creds, sheet_name: str, rows: list[list], folder_id: str | None = None
    ):
        ...
        spreadsheet = {"properties": {"title": sheet_name}}
        spreadsheet = (
            sheets_service.spreadsheets().create(body=spreadsheet, fields="spreadsheetId").execute()
        )
        spreadsheet_id = spreadsheet.get("spreadsheetId")
        ...
        try:
                ...
                requests.append(
                    {
                        "autoResizeDimensions": {
                            "dimensions": {
                                ...
                                "dimension": "COLUMNS",
                                "startIndex": 0,
                                "endIndex": min(10, len(rows[0]) if rows else 2),
                            }
                        }
                    }
                )
            ...
            current_app.logger.warning(
                f"Failed to apply formatting to spreadsheet {spreadsheet_id}: {e}"
            )
