...
class WordListService:
            ...
            for variant in variants:
                ...
                if len(tokens) > 1:
                    ingestion_report["multi_token_entries"].append(
                        {"original": variant, "head_token": self.extract_head_token(variant)}
                    )
                ...
                if not normalized:
                    ingestion_report["anomalies"].append(
                        {"word": raw_word, "issue": "empty_after_normalization"}
                    )
                    continue
                ...
                if normalized in normalized_keys:
                    ingestion_report["duplicates"].append(
                        {"word": variant, "normalized": normalized}
                    )
                else:
    ...
    def get_user_wordlists_query(user_id: int, include_global: bool = True):
        ...
        if include_global:
            ...
            query = WordList.query.filter(
                db.or_(WordList.owner_user_id == user_id, WordList.owner_user_id.is_(None))
            )
        else:
    ...
    def refresh_wordlist_from_source(
        self, wordlist: WordList, user=None, include_header: bool = True
    ) -> Dict:
        """
        ...
        if wordlist.words_json and len(wordlist.words_json) > 0:
            logger.info(
                f"WordList {wordlist.id} already has words_json with {len(wordlist.words_json)} words"
            )
            return {"status": "already_populated", "word_count": len(wordlist.words_json)}
        ...
        if wordlist.source_type == "google_sheet" and wordlist.source_ref:
            if not user or not user.google_access_token:
                raise ValueError(
                    "Google Sheets source requires authenticated user with Google access"
                )
            ...
            try:
                ...
                words = sheets_service.fetch_words_from_spreadsheet(
                    creds, spreadsheet_id=wordlist.source_ref, include_header=include_header
                )
                logger.info(f"Fetched {len(words)} words from Google Sheet {wordlist.source_ref}")
            except Exception as e:
                logger.exception(
                    f"Failed to refresh wordlist {wordlist.id} from Google Sheets: {e}"
                )
            ...
            logger.warning(
                f"WordList {wordlist.id} using canonical_samples ({len(words)} words) as fallback"
            )
        else:
        ...
        logger.info(
            f"Refreshed WordList {wordlist.id} with {len(normalized_keys)} normalized words"
        )
