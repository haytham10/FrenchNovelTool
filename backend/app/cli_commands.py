...
@click.option(
    "--description",
    "-d",
    default="Manual credit adjustment",
    help="Description for the credit transaction",
)
@with_appcontext
def add_credits_command(user_id, amount, description):
    ...
    try:
        ...
        credit_service.add_credits(user_id=user_id, amount=amount, description=description)
...
@click.option(
    "--description",
    "-d",
    default="Manual credit deduction",
    help="Description for the credit transaction",
)
@with_appcontext
def deduct_credits_command(user_id, amount, description):
    ...
    try:
        ...
        if user.credit_balance < amount:
            click.echo(
                click.style(
                    f"Warning: User only has {user.credit_balance:,} credits, but trying to deduct {amount:,}",
                    fg="yellow",
                )
            )
        ...
        # Deduct credits
        credit_service = CreditService()
        credit_service.deduct_credits(user_id=user_id, amount=amount, description=description)
