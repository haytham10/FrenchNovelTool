...
limiter = Limiter(key_func=_get_remote_address_for_limiter, default_limits=[])
socketio = SocketIO()
    ...
    try:
        with app.app_context():
            try:
                ...
                app.logger.warning(
                    "App will continue but users may not have a default wordlist available"
                )
    finally:
...
def configure_logging(app):
    ...
    if not app.debug and not app.testing:
        ...
        # File handler
        file_handler = RotatingFileHandler(log_file, maxBytes=10240000, backupCount=10)  # 10MB
