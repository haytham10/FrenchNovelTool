...
class ExportToSheetSchema(Schema):
    ...
    sheetName = fields.String(required=True, validate=validate.Length(min=1, max=255))
    folderId = fields.String(allow_none=True, validate=validate.Length(max=255))
    # P1 new fields
    mode = fields.String(validate=validate.OneOf(["new", "append"]), load_default="new")
    existingSheetId = fields.String(allow_none=True, validate=validate.Length(max=255))
    tabName = fields.String(allow_none=True, validate=validate.Length(max=255))
...
class UserSettingsSchema(Schema):
    ...
    sentence_length_limit = fields.Integer(required=True, validate=validate.Range(min=3, max=50))
    gemini_model = fields.String(
        load_default="speed", validate=validate.OneOf(["balanced", "quality", "speed"])
    )
    ...
    min_sentence_length = fields.Integer(load_default=2, validate=validate.Range(min=1, max=10))
...
class ProcessPdfOptionsSchema(Schema):
    ...
    sentence_length_limit = fields.Integer(validate=validate.Range(min=3, max=50), allow_none=True)
    gemini_model = fields.String(
        validate=validate.OneOf(["balanced", "quality", "speed"]), allow_none=True
    )
    ...
    min_sentence_length = fields.Integer(validate=validate.Range(min=1, max=10), allow_none=True)
...
class EstimateRequestSchema(Schema):
    ...
    model_preference = fields.String(
        required=True, validate=validate.OneOf(["balanced", "quality", "speed"])
    )
...
class WordListCreateSchema(Schema):
    ...
    source_type = fields.String(
        required=True, validate=validate.OneOf(["csv", "google_sheet", "manual"])
    )
...
class CoverageRunCreateSchema(Schema):
    ...
    mode = fields.String(required=True, validate=validate.OneOf(["coverage", "filter", "batch"]))
    source_type = fields.String(required=True, validate=validate.OneOf(["job", "history"]))
