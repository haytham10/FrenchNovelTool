...
def reconcile_stuck_chunks_endpoint():
    ...
    try:
        ...
        try:
            ...
            return jsonify({"message": "Reconciliation completed", "result": result}), 200
        except Exception:
            return jsonify({"message": "Reconciliation started", "task_id": task.id}), 202
...
def terminate_stuck_jobs_endpoint():
    ...
    try:
        ...
        try:
            ...
            return jsonify({"message": "Termination started", "task_id": task.id}), 202
...
def estimate_pdf():
    ...
    try:
        ...
        estimated_tokens = (page_count_for_calc * PAGES_TO_TOKENS_HEURISTIC) + (
            image_count * ESTIMATE_IMAGE_WEIGHT
        )
        ...
        return (
            jsonify({"error": "Invalid or corrupted PDF file", "details": str(e)}),
            HTTP_UNPROCESSABLE_ENTITY,
        )
    except Exception as e:
...
def process_pdf():
    ...
    try:
        ...
        sentence_length_limit = _coerce_int(
            form_data.get("sentence_length_limit"), settings.get("sentence_length_limit", 8)
        )
        gemini_model = (
            form_data.get("gemini_model") or settings.get("gemini_model", "speed")
        ).lower()
        if gemini_model not in {"balanced", "quality", "speed"}:
            gemini_model = "speed"
        ignore_dialogue = _coerce_bool(
            form_data.get("ignore_dialogue"), settings.get("ignore_dialogue", False)
        )
        preserve_formatting = _coerce_bool(
            form_data.get("preserve_formatting"), settings.get("preserve_formatting", True)
        )
        fix_hyphenation = _coerce_bool(
            form_data.get("fix_hyphenation"), settings.get("fix_hyphenation", True)
        )
        min_sentence_length = max(
            1,
            _coerce_int(
                form_data.get("min_sentence_length"), settings.get("min_sentence_length", 2)
            ),
        )
        ...
        current_app.logger.info(
            f"Successfully processed PDF: {original_filename}, extracted {len(processed_sentences)} sentences"
        )
        ...
        return jsonify({"sentences": processed_sentences, "job_id": job.id if job else None})

    except json.JSONDecodeError as e:
        error_message = f"Failed to parse Gemini response: {str(e)}"
        current_app.logger.exception(
            f"User {user.email} failed to process PDF {original_filename} due to JSON parsing error"
        )
...
def export_to_sheet():
    ...
    try:
        ...
        recent_entry = (
            History.query.filter_by(user_id=user_id, spreadsheet_url=None)
        ...
        return jsonify({"message": "Export successful", "spreadsheet_url": spreadsheet_url})
...
def export_history_to_sheets(entry_id):
    ...
    try:
        ...
        entry_detail = history_service.get_entry_with_details(
            entry_id, user_id, use_live_chunks=True
        )
        if not entry_detail:
            return jsonify({"error": "History entry not found"}), 404
        ...
        sheets_service = GoogleSheetsService()
        spreadsheet_url = sheets_service.export_to_sheet(
            creds=creds, sentences=prepared_sentences, sheet_name=sheet_name, folder_id=folder_id
        )
...
def get_job_status(job_id):
    ...
    if job.celery_task_id and job.status == JOB_STATUS_PROCESSING:
        ...
        task_state = {"state": task.state, "info": task.info if task.info else {}}
...
def cancel_job(job_id):
    ...
    return (
        jsonify({"message": "Job cancelled successfully", "job_id": job.id, "status": job.status}),
        200,
    )
...
def retry_failed_chunks(job_id):
    ...
    if not chunks_to_retry:
        return jsonify({"message": "No chunks eligible for retry", "retried_count": 0}), 200
    ...
    for chunk in chunks_to_retry:
        ...
        retry_tasks.append(process_chunk.s(chunk.get_chunk_metadata(), user_id, settings))
    ...
    current_app.logger.info(
        f"Job {job_id}: manually retrying {len(chunks_to_retry)} chunks, group_id={group_result.id}"
    )
        ...
        for source_type in ["csv", "google_sheet", "manual"]:
            for is_global in [True, False]:
                count = WordList.query.filter_by(
                    source_type=source_type, is_global_default=is_global
                ).count()
                wordlists_total.labels(
                    source_type=source_type, is_global=str(is_global).lower()
                ).set(count)

        # Count coverage assignments by mode
        for mode in ["coverage", "filter"]:
            # Get all completed runs for this mode
            completed_runs = CoverageRun.query.filter_by(mode=mode, status="completed").all()
