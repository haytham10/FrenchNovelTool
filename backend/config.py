...
class Config:
    ...
    CORS_ORIGINS = os.getenv(
        "CORS_ORIGINS",
        "http://localhost:3000,http://127.0.0.1:3000,https://www.frenchnoveltool.com,https://frenchnoveltool.com",
    )
    ...
    if redis_url.startswith("rediss://") or (
        redis_url.startswith("redis://") and os.getenv("REDIS_TLS", "false").lower() == "true"
    ):
    ...
    CHUNK_TASK_MAX_RETRIES = int(
        os.getenv("CHUNK_TASK_MAX_RETRIES", "4")
    )  # More retries with better resources
    CHUNK_TASK_RETRY_DELAY = int(os.getenv("CHUNK_TASK_RETRY_DELAY", "3"))  # Faster retries
    CHORD_WATCHDOG_SECONDS = int(
        os.getenv("CHORD_WATCHDOG_SECONDS", "300")
    )  # 5 min - more breathing room
    CHUNK_WATCHDOG_SECONDS = int(
        os.getenv("CHUNK_WATCHDOG_SECONDS", "600")
    )  # 10 min - handle large chunks
    # If a chunk remains 'processing' longer than this, it's likely stuck
    CHUNK_STUCK_THRESHOLD_SECONDS = int(
        os.getenv("CHUNK_STUCK_THRESHOLD_SECONDS", "720")
    )  # 12 minutes
    # Finalization Configuration
    FINALIZE_MAX_RETRIES = int(
        os.getenv("FINALIZE_MAX_RETRIES", "10")
    )  # More retries for complex jobs
    FINALIZE_RETRY_DELAY = int(os.getenv("FINALIZE_RETRY_DELAY", "15"))  # Faster checks

    # LLM Call Timeout (prevent indefinite hangs)
    GEMINI_CALL_TIMEOUT_SECONDS = int(
        os.getenv("GEMINI_CALL_TIMEOUT_SECONDS", "300")
    )  # 5 min for large chunks
