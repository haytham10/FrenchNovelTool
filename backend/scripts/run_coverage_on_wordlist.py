...
def write_learning_set_csv(out_dir, learning_set):
    ...
    with open(csv_path, "w", encoding="utf-8") as f:
        ...
        for item in learning_set:
            ...
            f.write(
                f"{item.get('rank')},{item.get('sentence_index')},{item.get('token_count')},{item.get('new_word_count',0)},{item.get('score',0)},\"{text}\"\n"
            )
    return csv_path
...
def main():
    ...
    parser.add_argument(
        "--sentences", "-s", required=True, help="Path to sentences file (JSON list or TXT lines)"
    )
    parser.add_argument(
        "--wordlist",
        "-w",
        default=os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "2k_list.json")),
        help="Path to wordlist JSON (default: backend/2k_list.json)",
    )
    parser.add_argument(
        "--out",
        "-o",
    ...
    parser.add_argument(
        "--min-tokens", type=int, default=4, help="Minimum token count to consider (default: 4)"
    )
    parser.add_argument(
        "--max-tokens", type=int, default=8, help="Maximum token count to consider (default: 8)"
    )
