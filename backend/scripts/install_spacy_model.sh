...
if python -m spacy download fr_core_news_sm --quiet 2>/dev/null; then
    echo "✅ Successfully downloaded fr_core_news_sm"
elif python -m spacy download fr_core_news_md --quiet 2>/dev/null; then
    echo "✅ Successfully downloaded fr_core_news_md (fallback)"
else
    echo "⚠️  Failed to download fr_core_news_sm and fr_core_news_md, trying fr_core_news_sm as fallback..."
    if python -m spacy download fr_core_news_sm --quiet 2>/dev/null; then
