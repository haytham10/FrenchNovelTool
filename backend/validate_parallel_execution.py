...
def test_merge_chunk_results():
    ...
    assert (
        normalized_texts.count("Overlap sentence.") == 1
    ), "Overlap sentence should appear only once"
...
def test_job_status_logic():
    ...
    chunk_results = [{"chunk_id": 0, "status": "success"}, {"chunk_id": 1, "status": "success"}]
    success_count = len([r for r in chunk_results if r.get("status") == "success"])
    ...
    chunk_results = [{"chunk_id": 0, "status": "success"}, {"chunk_id": 1, "status": "failed"}]
    success_count = len([r for r in chunk_results if r.get("status") == "success"])
    ...
    chunk_results = [{"chunk_id": 0, "status": "failed"}, {"chunk_id": 1, "status": "failed"}]
    success_count = len([r for r in chunk_results if r.get("status") == "success"])
