...
# Use inspector to make migrations defensive in case the DB already has these
# tables/columns (for example when running against a DB that was partially
# migrated or created out-of-band). This avoids DuplicateTable errors.
...
def upgrade():
    ...
    # Create word_lists table if it doesn't already exist
    if 'word_lists' not in inspector.get_table_names():
    ...
    # Create coverage_runs table if it doesn't already exist
    if 'coverage_runs' not in inspector.get_table_names():
    ...
    # Create coverage_assignments table if it doesn't already exist
    if 'coverage_assignments' not in inspector.get_table_names():
    ...
    # Extend user_settings table (only if columns don't already exist)
    user_settings_cols = [c['name'] for c in inspector.get_columns('user_settings')] if 'user_settings' in inspector.get_table_names() else []
    if 'default_wordlist_id' not in user_settings_cols:
        op.add_column('user_settings', sa.Column('default_wordlist_id', sa.Integer(), nullable=True))
        # create FK only if word_lists exists
        if 'word_lists' in inspector.get_table_names():
            op.create_foreign_key('fk_user_settings_default_wordlist', 'user_settings', 'word_lists', ['default_wordlist_id'], ['id'])
    if 'coverage_defaults_json' not in user_settings_cols:
...
def downgrade():
    ...
    if 'user_settings' in inspector.get_table_names():
        ...
        if 'default_wordlist_id' in cols:
            # drop FK if present
            try:
        ...
        if 'coverage_defaults_json' in cols:
            op.drop_column('user_settings', 'coverage_defaults_json')

    # Drop coverage_assignments table
    if 'coverage_assignments' in inspector.get_table_names():
        try:
    ...
    if 'coverage_runs' in inspector.get_table_names():
        for idx in [op.f('ix_coverage_runs_wordlist_id'), op.f('ix_coverage_runs_user_id'), op.f('ix_coverage_runs_status'), op.f('ix_coverage_runs_source_id'), op.f('ix_coverage_runs_mode'), op.f('ix_coverage_runs_celery_task_id')]:
            try:
                op.drop_index(idx, table_name='coverage_runs')
            except Exception:
                pass
        try:
