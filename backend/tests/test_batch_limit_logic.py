...
def test_batch_sentence_limit():
    ...
    for s in all_sentences:
        print(
            f"  ID={s['id']}, Text='{s['text']}', Score={s['quality_score']}, NewWords={s['new_word_count']}, Tokens={s['words']}"
        )
    ...
    # Verify
    assert (
        len(truncated) == target_count
    ), f"Expected {target_count} sentences, got {len(truncated)}"
