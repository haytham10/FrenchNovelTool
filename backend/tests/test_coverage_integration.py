...
class TestWordListAPIs:
    ...
    def test_create_wordlist_json(self, client, auth_headers):
        ...
        response = client.post("/api/v1/wordlists", headers=auth_headers, data=json.dumps(payload))
    ...
    def test_get_wordlist(self, client, auth_headers, sample_wordlist):
        ...
        response = client.get(f"/api/v1/wordlists/{sample_wordlist.id}", headers=auth_headers)
    ...
    def test_delete_wordlist(self, client, auth_headers, sample_wordlist):
        ...
        response = client.delete(f"/api/v1/wordlists/{sample_wordlist.id}", headers=auth_headers)

        assert response.status_code == 200

        # Verify it's deleted
        check_response = client.get(f"/api/v1/wordlists/{sample_wordlist.id}", headers=auth_headers)
...
class TestCoverageRunAPIs:
    ...
    def test_create_coverage_run(self, client, auth_headers, sample_wordlist, sample_history):
        ...
        response = client.post(
            "/api/v1/coverage/run", headers=auth_headers, data=json.dumps(payload)
        )
    ...
    def test_get_coverage_run(
        self, client, auth_headers, sample_user, sample_wordlist, sample_history
    ):
        ...
        response = client.get(f"/api/v1/coverage/runs/{coverage_run.id}", headers=auth_headers)
...
class TestCoverageServiceIntegration:
    ...
    def test_coverage_mode_end_to_end(self, app):
        ...
        sentences = ["Le chat mange.", "Le chien dort dans la maison.", "La maison est grande."]
    ...
    def test_filter_mode_end_to_end(self, app):
        ...
        config = {"min_in_list_ratio": 0.6, "len_min": 3, "len_max": 4, "target_count": 10}
...
class TestIngestionReporting:
    ...
    def test_ingestion_report_duplicates(self, app, sample_user):
        ...
        wordlist, report = service.ingest_word_list(
            words=words, name="Duplicate Test", owner_user_id=sample_user.id, source_type="manual"
        )
    ...
    def test_ingestion_report_variants(self, app, sample_user):
        ...
        wordlist, report = service.ingest_word_list(
            words=words, name="Variant Test", owner_user_id=sample_user.id, source_type="manual"
        )
    ...
    def test_ingestion_report_multi_token(self, app, sample_user):
        ...
        wordlist, report = service.ingest_word_list(
            words=words, name="Multi-token Test", owner_user_id=sample_user.id, source_type="manual"
        )
