...
class TestChunkingServicePersistence:
    ...
    @patch("builtins.open")
    def test_split_pdf_and_persist_creates_chunks(
        self, mock_open, mock_pdf_writer, mock_pdf_reader
    ):
        ...
        chunk_ids = service.split_pdf_and_persist(
            pdf_path="/tmp/test.pdf", chunk_config=chunk_config, job_id=123, db=mock_db
        )
    ...
    def test_split_pdf_and_persist_sets_chunk_metadata(self, mock_open, mock_pdf_reader):
        ...
        chunk_ids = service.split_pdf_and_persist(
            pdf_path="/tmp/test.pdf", chunk_config=chunk_config, job_id=456, db=mock_db
        )
...
class TestChunkStatusTracking:
    ...
    def test_chunk_status_transitions(self):
        ...
        chunk = JobChunk(
            job_id=1, chunk_id=0, start_page=0, end_page=10, page_count=10, status="pending"
        )
    ...
    def test_chunk_error_tracking(self):
        ...
        chunk = JobChunk(
            job_id=1, chunk_id=0, start_page=0, end_page=10, page_count=10, status="processing"
        )
...
class TestChunkResultPersistence:
    ...
    def test_chunk_result_json_storage(self):
        ...
        chunk = JobChunk(
            job_id=1, chunk_id=0, start_page=0, end_page=10, page_count=10, status="success"
        )
