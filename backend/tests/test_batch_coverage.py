...
class TestBatchCoverageMode:
    ...
    def test_batch_coverage_basic(self, app):
        ...
        service = CoverageService(
            wordlist_keys=wordlist_keys, config={"len_min": 3, "len_max": 10, "target_count": 0}
        )
    ...
    def test_batch_coverage_sequential_reduction(self, app):
        ...
        source2 = ["Cinq enfants jouent.", "Six livres sont là."]

        # Source 3: Contains sept, huit
        source3 = ["Sept jours par semaine.", "Huit heures du matin."]

        service = CoverageService(
            wordlist_keys=wordlist_keys, config={"len_min": 2, "len_max": 10, "target_count": 0}
        )

        sources = [(1, source1), (2, source2), (3, source3)]
    ...
    def test_batch_coverage_vs_single_coverage(self, app):
        ...
        # Novel A: maison, chat
        novel_a = ["La maison est belle.", "Le chat est noir."]

        # Novel B: livre, table
        novel_b = ["Le livre est bon.", "La table est ronde."]
    ...
    def test_batch_coverage_respects_sentence_limit(self, app):
        ...
        assert (
            len(learning_set) <= sentence_limit
        ...
        if len(learning_set) > 1:
            for i in range(len(learning_set) - 1):
                ...
                assert (
                    learning_set[i]["score"] >= learning_set[i + 1]["score"]
    ...
    def test_batch_coverage_metadata_completeness(self, app):
        ...
        source1 = ["Le chat est sur la maison.", "Le chien court vite."]

        service = CoverageService(
            wordlist_keys=wordlist_keys, config={"len_min": 2, "len_max": 10, "target_count": 10}
        )
